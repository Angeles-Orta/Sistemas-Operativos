/*
 * Versión portable para máxima compatibilidad
 * Usar si la versión principal da errores de compilación
 */
 ...
 // Variables globales para getopt (si no están definidas)
 #ifndef _GETOPT_H
 extern char *optarg;
 ...
 void process_worker(shared_data_t* shared_data, int worker_id, int increments, int use_sync) {
     printf("[PROCESO %d] Iniciando %d incrementos%s\n", 
            worker_id, increments, use_sync ? " (con sincronización)" : " (sin sincronización)");
     ...
     printf("[PROCESO %d] Completado. Contador local: %d\n", worker_id, shared_data->counter);
 }
 ...
 void* thread_worker(void* arg) {
     ...
     printf("[HILO %d] Iniciando %d incrementos%s\n", 
            args->worker_id, args->increments, args->use_mutex ? " (con mutex)" : " (sin sincronización)");
 ...
 void test_threads(int num_workers, int increments_per_worker, int use_sync) {
     ...
     thread_args_t* args = malloc(num_workers * sizeof(thread_args_t));
 ...
 int main(int argc, char* argv[]) {
     ...
     // Procesar argumentos de línea de comandos usando getopt manual si es necesario
     int opt;
